...
\usepackage{pmmeta}
\pmcanonicalname{LaplacianFromRectangularToSphericalCoordinates1}
\pmcreated{2013-03-11 19:24:44}
\pmmodified{2013-03-11 19:24:44}
\pmowner{swapnizzle}{13346}
\pmmodifier{}{0}
\pmtitle{Laplacian from Rectangular to Spherical Coordinates}
\pmrecord{1}{50076}
\pmprivacy{1}
...
\headheight 0in
\headsep 0in
\title{The Laplacian from Rectangular to Spherical Coordinates}
\author{Swapnil Sunil Jain}
\date{June 4, 2006}
\begin{document}
\maketitle
We begin by recognizing the familiar conversion from rectangular to spherical coordinates\footnote[1]{Note that convention in use here: $\phi$ is the azimuthal angle, whereas $\theta$ is the polar angle.}
\begin{eqnarray}
...
\end{eqnarray}
Now, we know that the Laplacian in rectangular coordinates is defined in the following way\footnote[2]{Readers should note that we do not have to define the Laplacian this way. A more rigorous approach would be define the Laplacian using a coordinate-free expression.} 
\begin{eqnarray}
...
\end{eqnarray}
The next step is to manipulate the right-hand side of each of the above three equations so that it is only a function of $r$, $\theta$ and $\phi$. We can do this by substituting the following expressions in their respective places in the above three equations\footnote[3]{These expression can be derived simply by taking the proper derivative of the expressions in (2) and then substituting the expressions from (1) in order to eliminate any dependence on $x$, $y$, and $z$.}
\begin{eqnarray}
...
\end{eqnarray}
Assuming that $f$ is a sufficiently differentiable function, we can replace $f$ by $\frac{\partial f}{\partial x}$ and arrive at the following
\begin{eqnarray}
...
\end{eqnarray}
We then differentiate both sides with respect to $y$ and arrive at the following
\begin{eqnarray}
...
\end{eqnarray}
We then differentiate both sides of the above equation with respect to $z$ resulting in the following 
\begin{eqnarray}
